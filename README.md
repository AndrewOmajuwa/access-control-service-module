...
2. Set the environmental variables in the docker-compose.yml configuration namely;
- ${USER}: "A username for the MySql database"
- ${PASSWORD}: "A password for the MySql database"
...
3Open your terminal and run ```docker-compose up``` to download the relevant container images and start mysql database and keycloak server.
...
6. Set the following environmental variables in the access-control-service configuration;
...
7. Go to Postman and execute a POST request to ```http://localhost:8180/realms/${REALM_NAME}/protocol/openid-connect/token``` to generate an access token. Send the following details with the request; client_id, client_secret, grant_type (password), username, password and scope (openid).
