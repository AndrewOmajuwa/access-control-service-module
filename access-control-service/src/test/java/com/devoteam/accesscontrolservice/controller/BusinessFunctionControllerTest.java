...
class BusinessFunctionControllerTest {
    ...
    @Test
    @DisplayName("cascade delete removes a business function and all its associations when successfully executed")
    void delete_RemovesABusinessFunctionAndAllAssociations_WhenSuccessfullyExecuted(){

        ResponseEntity<Void> responseEntity = testRestTemplate.exchange("/api/v1/business-functions/1/cascade", HttpMethod.DELETE, null, Void.class);

        Assertions.assertThat(responseEntity).isNotNull();

        Assertions.assertThat(responseEntity.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);

        Assertions.assertThat(businessFunctionRepository.findById(1)).isEmpty();
    ...
    @Test
    @DisplayName("cascade delete business function returns 404 ResourceNotfound when business function id does not exist")
    void cascadeDeleteBusinessFunction_Returns404ResourceNotFound_WhenBusinessFunctionIdDoesNotExist(){


        ResponseEntity<Void> responseEntity = testRestTemplate.exchange("/api/v1/business-functions/100/cascade", HttpMethod.DELETE, null, Void.class);
    ...
    cascadeDelete_rollsBackTransaction_whenExceptionIsThrown() throws RuntimeException{
        ...
        BDDMockito.doThrow(new RuntimeException("Exception message")).when(businessFunctionRepository).delete(ArgumentMatchers.any());

        testRestTemplate.exchange("/api/v1/business-functions/1/cascade", HttpMethod.DELETE, null, Void.class);

        Assertions.assertThat(businessFunctionRepository.findById(1)).isPresent();
